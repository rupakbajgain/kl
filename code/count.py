...
c={}
total=[0,0]
csv_file = open('count.csv', 'w', newline='')
...
for i in range(275):
    try:
        with open(str(i+1)+'.txt',"r") as f:
            ...
            r=d['records']
            for i in r:
                if(i['contest']=="OS Nepal Music Award 2076"):
                    if(not c.get(i["contestant"]["name"])):
                       c[i["contestant"]["name"]]=[0,0]
                    if(i['vote_count']==1):
                        c[i["contestant"]["name"]][0]=c[i["contestant"]["name"]][0]+int(i['vote_count'])
                        total[0]=total[0]+int(i['vote_count'])
                    else:
                        c[i["contestant"]["name"]][1]=c[i["contestant"]["name"]][1]+int(i['vote_count'])
